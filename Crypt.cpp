...
{
    PKeyCtxPtr ctx(
        EVP_PKEY_CTX_new_from_name(nullptr, "X25519", nullptr),
        &EVP_PKEY_CTX_free);
    if (!ctx)
        throw_openssl_error("EVP_PKEY_CTX_new_from_name failed");
    ...
    // Private key, PKCS#8 PEM ("BEGIN PRIVATE KEY")
    {
        ...
        if (!bio)
            throw_openssl_error("BIO_new_file (private key) failed");
    ...
    // Public key PEM ("BEGIN PUBLIC KEY")
    {
        ...
        if (!bio)
            throw_openssl_error("BIO_new_file (public key) failed");
        if (PEM_write_bio_PUBKEY(bio.get(), key.get()) != 1)
            throw_openssl_error("PEM_write_bio_PUBKEY failed");
    }

    // Also keep the public key text so the app can show it to the user
    {
        BioPtr mem(BIO_new(BIO_s_mem()), &BIO_free);
        if (!mem)
            throw_openssl_error("BIO_new (memory) failed");
        if (PEM_write_bio_PUBKEY(mem.get(), key.get()) != 1)
            throw_openssl_error("PEM_write_bio_PUBKEY (memory) failed");
    ...
    std::size_t aad_length)
{
    ...
    validate_optional_buffer(plaintext, plaintext_length, "plaintext");
...
// ---- 热路径零拷贝实现（与服务端 server/Crypt/crypt.cpp 同一算法布局）----
// EVP_CIPHER_CTX 线程本地复用 + 逐包 EVP_CIPHER_CTX_reset，
// 输出（nonce||ct||tag）直写调用方缓冲区，逐包 0 次堆分配
    ...
    std::uint8_t* out, std::size_t out_cap)
{
    ...
    if (out == nullptr || out_cap < pt_len + AES_GCM_NONCE_LEN + AES_GCM_TAG_LEN) {
    ...
    std::uint8_t* out, std::size_t out_capacity)
{
    ...
    if (sealed == nullptr || sealed_length < overhead || out == nullptr) {
        return 0;
    }
    ...
    std::string& pub_pem_out)
{
    PKeyCtxPtr ctx(
        ...
        &EVP_PKEY_CTX_free);
    if (!ctx)
        throw_openssl_error("EVP_PKEY_CTX_new_from_name(ED25519) failed");
    ...
    // 私钥：PKCS#8 PEM
    {
        BioPtr bio(BIO_new_file(priv_path.c_str(), "wb"), &BIO_free);
        if (!bio)
            throw_openssl_error("BIO_new_file (Ed25519 private key) failed");
        if (PEM_write_bio_PrivateKey(
                bio.get(), key.get(), nullptr, nullptr, 0, nullptr, nullptr) != 1)
            throw_openssl_error("PEM_write_bio_PrivateKey failed");
    }
    ...
    // 公钥：SPKI PEM
    {
        BioPtr bio(BIO_new_file(pub_path.c_str(), "wb"), &BIO_free);
        if (!bio)
            throw_openssl_error("BIO_new_file (Ed25519 public key) failed");
        if (PEM_write_bio_PUBKEY(bio.get(), key.get()) != 1)
            throw_openssl_error("PEM_write_bio_PUBKEY failed");
    }
...
EVP_PKEY* load_ed25519_private_key(const std::string& pem_path)
{
    ...
    if (pem_data.empty()) {
        throw std::runtime_error(
            "Ed25519 private-key file is empty: " + pem_path);
    }
...
EVP_PKEY* load_ed25519_public_key(const std::string& pem_path)
{
    ...
    if (pem_data.empty()) {
        throw std::runtime_error(
            "Ed25519 public-key file is empty: " + pem_path);
    }
...
// Ed25519 密钥：从 PEM 文本加载
EVP_PKEY* load_ed25519_private_key_pem(const std::string& pem_text)
{
    if (pem_text.empty()) {
        throw std::invalid_argument(
            "Ed25519 private key PEM is empty");
    }

    BioPtr bio(
        BIO_new_mem_buf(
            pem_text.data(),
            checked_size_to_int(pem_text.size(), "pem_text")),
        &BIO_free);
...
EVP_PKEY* load_ed25519_public_key_pem(const std::string& pem_text)
{
    if (pem_text.empty()) {
        throw std::invalid_argument(
            "Ed25519 public key PEM is empty");
    }
